...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/amahman/lol/master/gerung > /root/tmp
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Tikusmerdeka/amahman/lol/gerung | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
